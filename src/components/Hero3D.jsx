  ...
  return (
    <section className="relative min-h-screen flex items-center overflow-hidden">
      <div className="absolute inset-0">
        <Canvas camera={{ position: [0, 0, 6], fov: 45 }} dpr={perf.tier === 'low' ? [1, 1.5] : [1, 2]} gl={{ powerPreference: 'high-performance', antialias: perf.tier === 'high' }}>
          <Suspense fallback={null}>
            <Scene />
          </Suspense>
        </Canvas>
      ...
      <div className="relative z-10 max-w-7xl mx-auto px-4 sm:px-6 pt-24 pb-20 w-full">
        <div className="max-w-3xl">
          ...
          <motion.h1
            {...fadeIn}
            transition={perf.reducedMotion ? {} : { duration: 0.8, delay: 0.15, ease: 'easeOut' }}
          ...
          <motion.p
            {...fadeIn}
            transition={perf.reducedMotion ? {} : { duration: 0.8, delay: 0.3, ease: 'easeOut' }}
            className="mt-6 text-base sm:text-lg md:text-xl text-gray-400 max-w-xl leading-relaxed"
          >
            Pulsemation builds custom AI automation pipelines that eliminate busywork,
            accelerate operations, and scale with your business.
          </motion.p>

          <motion.div
            {...fadeIn}
            transition={perf.reducedMotion ? {} : { duration: 0.8, delay: 0.45, ease: 'easeOut' }}
      ...
      {!perf.reducedMotion && (
        ...
        >
          <a href="#services" className="flex flex-col items-center gap-2 text-gray-600 hover:text-gray-400 transition-colors">
            <span className="text-xs tracking-widest uppercase">Scroll</span>
            <svg width="16" height="24" viewBox="0 0 16 24" className="animate-bounce">
              <rect x="1" y="1" width="14" height="22" rx="7" stroke="currentColor" strokeWidth="2" fill="none" />
